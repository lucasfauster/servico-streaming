...
class ClientTCP:
    ...
    def has_permission(self, message):
        if message == "PERMISSAO_NEGADA":
            self.log.error("method = has_permission, message = Only premium users can do this action!")
            return False
        return True
    ...
    def log_in(self, name, user_type):
        ...
        if resp[0] == "ENTRAR_NA_APP_ACK":
            ...
            return True
        self.log.error("method = log_in, message = An error occurred while entering the app!")
        return False
    ...
    def create_group(self):
        ...
        if self.has_permission(resp[0]):
            if resp[0] == "CRIAR_GRUPO_ACK":
                ...
                self.log.error("method = create_group, message = An error occurred while creating group!")
    ...
    def add_to_group(self, chosen_user):
        ...
        if self.has_permission(resp[0]):
            if resp[0] == "ADD_USUARIO_GRUPO_ACK":
                ...
                self.log.error("method = add_to_group, message = An error occurred while adding user to group!")
                return False
    ...
    def get_group(self):
        ...
        if resp[0] == "GRUPO_DE_STREAMING":
            ...
            self.log.error("method = get_group, message = An error occurred while fetching group: {}".format(resp[1]))
            return None
    ...
    def remove_from_group(self, chosen_user):
        ...
        if self.has_permission(resp[0]):
            if resp[0] == "REMOVER_USUARIO_GRUPO_ACK":
                ...
                self.log.error("method = add_to_group, message = An error occurred while removing user from group!")
                return False
